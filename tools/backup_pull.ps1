...
$defaultDest = Join-Path ([Environment]::GetFolderPath('MyDocuments')) 'PandaTouch_backups'
...
function Pick-Folder($startAt) {
    ...
    try {
        ...
        $dlg.Description         = 'Kies waar de PandaTouch back-ups moeten komen'
...
if ($text -notmatch '^#PTBACKUP1' -or $text -notmatch '@@spools') {
    Write-Host "MISLUKT: onverwacht antwoord (geen PandaTouch back-up) - niets weggeschreven."
    exit 1
}
