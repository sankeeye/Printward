...
1. **App holen.** Öffne auf dem Tablet einen Webbrowser und gehe zur
   [**Releases-Seite**](https://github.com/sankeeye/FilaTrack/releases). Lade **`FilaTrack.apk`**
   herunter. *(Oder lade sie auf einem Computer herunter und kopiere sie per USB aufs Tablet.)*

2. **Installieren.** Tippe auf die heruntergeladene `FilaTrack.apk`. Android warnt, dass es
...
3. **Ersteinrichtung.** Öffne FilaTrack. Es fragt nach deinem Drucker:
...
Nur wenn du die **FilaTrack Scale** hast (ESP32-S3 + Wägezelle). Sie wiegt deine Spulen, damit
...
4. **Ersteinrichtung.** Nach dem Flashen erstellt die Waage ihr eigenes WLAN-Netzwerk. Verbinde
   ...
   unter **Einstellungen ▸ FilaTrack Scale** ein.
