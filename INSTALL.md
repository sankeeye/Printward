...
Printward Scale hardware. No developer tools, no building.
...
1. **Get the app.** On the tablet, open a web browser and go to the
   [**Releases page**](https://github.com/sankeeye/Printward/releases). Download
   **`Printward.apk`**. *(Or download it on a computer and copy it to the tablet
   over USB.)*

2. **Install it.** Tap the downloaded `Printward.apk`. Android will warn that it
...
3. **First-time setup.** Open Printward. It asks for your printer:
...
4. **The web page (optional).** You can also control the printer from any browser on
   ...
   **Settings ▸ Web page** and **Settings ▸ Web password** (each tablet makes its own
   password — sign in as user `printward`).
...
Only if you have the **Printward Scale** (ESP32-S3 + load cell). It weighs your spools
...
4. **First-time setup.** After flashing, the scale makes its own Wi-Fi network. Connect
   ...
   under **Settings ▸ Printward Scale**.
