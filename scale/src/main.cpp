// Printward Scale - firmware for the (SpoolEase) load-cell scale hardware.
//
// An ESP32-S3 + HX711 that reports the weight on the platform to the Printward
...
// if it can't connect it falls back to an open AP "Printward Scale-setup" so it's
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Printward Scale</title>
...
<h1>Printward Scale</h1><div class="box">
...
void setup() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        g_ap_mode = true;
        WiFi.mode(WIFI_AP);
        WiFi.softAP("Printward Scale-setup");
        Serial.print("Printward Scale AP-modus: http://");
        Serial.println(WiFi.softAPIP());
    } else {
        Serial.print("Printward Scale klaar. IP: ");
        Serial.println(WiFi.localIP());
        if (MDNS.begin("printward-scale")) MDNS.addService("http", "tcp", 80);
    }
