# Builds the PandaTouch UI simulator and launches it.
...
Write-Host "Building PandaTouch simulator..." -ForegroundColor Cyan
& $bash "$msys/build.sh"
...
Get-Process pandatouch_sim -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Milliseconds 300
Start-Process -FilePath (Join-Path $sim 'pandatouch_sim.exe') -WorkingDirectory $sim
